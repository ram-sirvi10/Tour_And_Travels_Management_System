...
<%@ page import="java.util.*, com.travelmanagement.dto.responseDTO.UserResponseDTO"%>
...
<div class="dashboard-container">
    ...
    <div class="main-content">
        ...
        <div class="mb-3">
            <form method="post" action="<%=request.getContextPath()%>/admin" class="row g-2">
                <input type="hidden" name="button" value="<%=request.getParameter("button")%>" />
                <% if (!"Deleted Users".equalsIgnoreCase(currentList)) { %>
                    <div class="col-md-2">
                        <select name="active" class="form-select">
                            <option value="">All</option>
                            <option value="true" <%="true".equals(request.getParameter("active")) ? "selected" : ""%>>Active</option>
                            <option value="false" <%="false".equals(request.getParameter("active")) ? "selected" : ""%>>Inactive</option>
                        </select>
                    </div>
                    <div class="col-md-2">
        ...
        <tr>
            ...
            <td>
                <form method="post" action="<%=request.getContextPath()%>/admin" style="display:inline;">
                    <input type="hidden" name="button" value="userAction">
                    <input type="hidden" name="userId" value="<%=user.getUserId()%>">
                    <input type="hidden" name="keyword" value="<%= request.getParameter("keyword") != null ? request.getParameter("keyword") : "" %>">
                    <input type="hidden" name="active" value="<%= request.getParameter("active") != null ? request.getParameter("active") : "" %>">
                    <input type="hidden" name="pageSize" value="<%= pageSize %>">

                    <button type="submit" name="action" value="<%=user.isActive() ? "deactivate" : "activate"%>" 
