...
.registration-container {min-height: 100vh; display: flex; justify-content: center; align-items: flex-start; padding: 60px 0;}
.registration-card {background: #fff; padding: 35px 30px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.2); width: 100%; max-width: 900px; transition: transform 0.3s ease, box-shadow 0.3s ease;}
.registration-card:hover {transform: translateY(-5px); box-shadow: 0 15px 35px rgba(0,0,0,0.3);}
.registration-card h2 {text-align: center; margin-bottom: 25px; font-weight: 700;}
.profile-upload-wrapper {display: flex; flex-direction: column; align-items: center; margin-bottom: 15px;}
.profile-preview {width: 120px; height: 120px; border-radius: 50%; border: 2px dashed #ccc; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; transition: border-color 0.3s, transform 0.2s; margin-bottom: 10px;}
.profile-preview:hover {border-color: #0d6efd; transform: scale(1.05);}
.preview-img {width: 120px; height: 120px; border-radius: 50%; object-fit: cover;}
.fade-alert {transition: opacity 0.5s ease;}
...
<input type="text" name="agency_name" class="form-control mb-2" placeholder="Agency Name"
       value="<%=request.getParameter("agency_name") != null ? request.getParameter("agency_name") : ""%>" <%=lockFields ? "readonly" : ""%>>
...
<input type="text" name="owner_name" class="form-control mb-2" placeholder="Owner Name"
       value="<%=request.getParameter("owner_name") != null ? request.getParameter("owner_name") : ""%>" <%=lockFields ? "readonly" : ""%>>
...
<div class="col-md-6">
    ...
    <select id="area" name="area" class="form-select mb-2" <%=lockFields ? "disabled" : ""%>></select>
    <% if (errors != null && errors.get("area") != null) { %><div class="error"><%=errors.get("area")%></div><% } %>

    <label class="form-label">Pincode <span style="color:red;">*</span></label>
    <input type="text" id="pincode" name="pincode" class="form-control mb-2" readonly value="<%=pincodeVal%>">
...
<p>Already have an account? <a href="login.jsp?role=agency">🏢 Login as Agency</a></p>
<% } else { %>
<div class="mt-3">
